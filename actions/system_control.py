...
class SystemController:
    ...
    def open_app(self, app_name: str) -> bool:
        ...
        try:
            logger.info(f"Ouverture de '{app_name}' : {app_path}")
            ...
            logger.info(f"Application '{app_name}' lancée avec succès")
            return True
    ...
    def close_app(self, app_name: str) -> bool:
        ...
        Returns:
            True si au moins un processus fermé, False sinon
        """
        # Trouver la meilleure correspondance
        matched_app = self._find_best_app_match(app_name)
        ...
        if not process_names:
            logger.warning(f"Aucun nom de processus trouvé pour '{app_name}'")
            return False
        ...
        try:
            ...
            if closed_count > 0:
                logger.info(f"Application '{app_name}' fermée ({closed_count} processus)")
                return True
            else:
                logger.warning(f"Aucun processus trouvé pour '{app_name}'")
                return False
                
        except Exception as e:
            logger.error(f"Erreur lors de la fermeture de '{app_name}' : {e}")
            return False
    ...
    def web_search(self, query: str) -> bool:
        """
        Ouvre une recherche web dans le navigateur par défaut.
